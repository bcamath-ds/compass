...
/*    EXPORTED FUNCTIONS:                                                   */
/*                                                                          */
/*  int CCedgegen_x_k_nearest (int ncount, int k, compass_data *dat,         */
...
/*      compass_data *dat, double *wcoord, int wantlist, int *ecount,        */
...
/*      compass_data *dat, int *outcycle, double *val)                       */
...
/*                                                                          */
/*  int CCedgegen_junk_k_nearest (int ncount, int k, compass_data *dat,      */
/*      double *wcoord, int wantlist, int *ecount, int **elist,             */
/*      int silent)                                                         */
/*    RETURNS the k-nearest graph (for JUNK-Norms)                          */
/*      -see CCedgegen_x_k_nearest (above) for the variables                */
/*                                                                          */
/*  int CCedgegen_junk_node_k_nearest (compass_data *dat, double *wcoord,    */
/*      int n, int k, int ncount, int *list)                                */
/*    RETURNS the k nearest neighbors to node n (for JUNK-Norms)            */
/*      -list returns the neighbors of n. The calling routine should        */
/*       be sure that list points to an array of length at least num.       */
/*                                                                          */
/*  int CCedgegen_junk_node_nearest (compass_data *dat, double *wcoord,      */
...
/*      compass_data *dat, int *outcycle, double *val, int silent)           */
...
/*                                                                          */
/*  int CCedgegen_junk_greedy_tour (int ncount, compass_data *dat,           */
...
/*       for large problems).                                               */
/*                                                                          */
/*  int CCedgegen_junk_qboruvka_tour (int ncount, compass_data *dat,         */
/*      int *outcycle, double *val, int ecount, int *elist, int silent)     */
...
/*                                                                          */
/*  int CCedgegen_xnear_build (int ncount, compass_data *dat,                */
...
static void
    ...
    insert (int n, int m, shortedge *nearlist, compass_data *dat,
        double *wcoord),
    x_quicksort (int *list, double *x, int l, int u);
static int
    run_x_k_nearest (int ncount, int num, compass_data *dat, double *wcoord,
...
int CCedgegen_x_k_nearest (int ncount, int num, compass_data *dat,
...
int CCedgegen_x_quadrant_k_nearest (int ncount, int num, compass_data *dat,
        double *wcoord, int wantlist, int *ecount, int **elist, int silent)
{
    return run_x_k_nearest (ncount, num, dat, wcoord, wantlist, ecount,
                            elist, 1, silent);
...
int CCedgegen_junk_k_nearest (int ncount, int num, compass_data *dat,
        double *wcoord, int wantlist, int *ecount, int **elist, int silent)
{
...
static int run_x_k_nearest (int ncount, int num, compass_data *dat,
        ...
        int silent)
{
    ...
    td.table = xcalloc (ncount, sizeof(intptr *));
    if (!td.table) {
        rval = 1;
        goto CLEANUP;
    }
    ...
    if (wantlist) {
        int j = 0;
        *elist = xcalloc (2 * ntotal, sizeof(int));
        if (!(*elist)) {
            rval = 1;
            goto CLEANUP;
        }
...
static void insert (int n, int m, shortedge *nearlist, compass_data *dat,
                    double *wcoord)
{
    int i;
    int thisdist;
...
int CCedgegen_junk_node_k_nearest (compass_data *dat, double *wcoord, int n,
        int nearnum, int ncount, int *list)
{
    int i, j, ntotal;
    shortedge *nearlist = (shortedge *) NULL;

    nearlist = xalloc (nearnum + 1, sizeof(shortedge));
    if (!nearlist)
        return 1;
    for (i = 0; i < nearnum; i++)
        nearlist[i].length = BIGDOUBLE;
...
int CCedgegen_junk_node_nearest (compass_data *dat, double *wcoord, int ncount,
        int n, char *marks)
{
    int j, bestnode = 0;
...
int CCedgegen_x_nearest_neighbor_tour (int ncount, int start, compass_data *dat,
                             int *outcycle, double *val)
{
    ...
    */
    ...
    marks = xcalloc (ncount, sizeof(char) );
    if (!marks) {
        CCedgegen_xnear_free (&xn);
        return 1;
    }
...
int CCedgegen_x_greedy_tour (int ncount, compass_data *dat, int *outcycle,
        double *val, int ecount, int *elist, int silent)
{
    ...
    perm = xcalloc (ecount, sizeof(int));
    elen = xcalloc (ecount, sizeof(int));
    if (perm == (int *) NULL ||
        elen == (int *) NULL) {
    ...
    if (outcycle) {
        tcyc = xcalloc (2 * ncount, sizeof(int));
        if (!tcyc) {
            rval = 1;
            goto CLEANUP;
        }
    ...
    degree = xcalloc (ncount, sizeof(int));
    if (!degree) {
        rval = 1;
        goto CLEANUP;
    }
    tail = xcalloc (ncount, sizeof(int));
    if (!tail) {
        rval = 1;
        goto CLEANUP;
    }
...
int CCedgegen_x_qboruvka_tour (int ncount, compass_data *dat, int *outcycle,
        double *val, int ecount, int *elist, int silent)
{
    int rval;
    int *perm = (int *) NULL;
    ...
    deg  = xcalloc (ncount, sizeof(int));
    adj  = xcalloc (ncount, sizeof(int *));
    adjspace = xcalloc (ecount*2, sizeof(int));
    if (deg  == (int *) NULL ||
        adj  == (int **) NULL ||
        adjspace == (int *) NULL) {
        fprintf (stderr, "Out of memory in CCedgegen_x_greedy_tour\n");
...
int CCedgegen_junk_greedy_tour (int ncount, compass_data *dat, int *outcycle,
        double *val, int ecount, int *elist, int silent)
{
    int rval;
    int *perm = (int *) NULL;
...
int CCedgegen_junk_qboruvka_tour (int ncount, compass_data *dat, int *outcycle,
        double *val, int ecount, int *elist, int silent)
{
    int rval;
    int *perm = (int *) NULL;
...
int CCedgegen_xnear_build (int ncount, compass_data *dat, double *wcoord,
        CCxnear *xn)
{
    int i;
    int norm;
    ...
    xn->nodenames = xcalloc (ncount, sizeof(int));
    if (!xn->nodenames)
        return 1;
    for (i = 0; i < ncount; i++)
        xn->nodenames[i] = i;
    ...
    xn->invnames = xcalloc (ncount, sizeof(int));
    if (!xn->invnames) {
        xfree (xn->nodenames);
    ...
    if (wcoord != (double *) NULL) {
        //xn->w = xcalloc (ncount, sizeof(double));
